...
//Archivos
void LeerArchivoLibros(TaLibros, int *);
void EscribirArchivoLibros(TaLibros, int);
...
void MenuLibros(){
	...
	LeerArchivoLibros(aLibros, &eTotal);
	...
	EscribirArchivoLibros(aLibros, eTotal);
...
/*
	Última actualización: 12/6/19
	Modificaciones: cambia el método de generación porque la información se almacena en archivos.
*/
int GenerarIdLibro (){
...
*/	
	...
	FILE *fIdLibros;
 	...
 	if((fIdLibros = fopen("IdLibros.dat", "r+")) == NULL){	 			  

		printf("No se pudo abrir el archivo de ids de libros.");
		getchar();
		
	} else {
	
		fflush(stdin);
		fscanf(fIdLibros, "%d", &eSiguiente);
		eSiguiente++;
		
		fflush(stdin);
		rewind(fIdLibros);
		fprintf(fIdLibros, "%d", eSiguiente);
...
void LeerArchivoLibros(TaLibros aLibros, int *eTotal){
	
	FILE *fLibros;
	TrLibro rLibro;
	 
 	if((fLibros = fopen("Libros.dat", "rb+")) != NULL){	 			  

		rewind(fLibros);
		while (! feof(fLibros)){
			  
			  fread (&rLibro, sizeof(rLibro), 1, fLibros);			  
		   	  InsertarLibro(aLibros, eTotal, rLibro);		   	  
		}
		fclose(fLibros);
	 	...
	 	printf("No se pudo abrir el archivo de libros para leer");
    }
}

void EscribirArchivoLibros(TaLibros aLibros, int eTotal){

	FILE *fLibros;
	int ePos;
	TrLibro rLibro;
	 
 	if((fLibros = fopen("Libros.dat", "wb+")) == NULL){
	 
	 	printf("No se pudo abrir el archivo de libros para escribir");
	 	fflush(stdin);
        getchar();
 	    ...
 	    for (ePos = 0; ePos <= eTotal; ePos++){
			...
			rLibro = aLibros[ePos];
			fflush(stdin);
			fwrite(&rLibro, sizeof(rLibro), 1, fLibros);
		}
		fclose(fLibros);
	}
}
